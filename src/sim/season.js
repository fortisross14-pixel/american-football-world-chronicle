...
export const OFFSEASON_STAGES=['Coach Market','College Transfers','Retirements & Declarations','Hall of Fame & Legacy','Free Agency','Draft','Trades & UFL','Development & New Class'];
...
export function ensureUniverse(universe){
  ...
  if(u.offseasonState){u.offseasonState.events=u.offseasonState.events||{};for(const k of ['coachMarket','transfers','retirements','declarations','hallOfFame','retiredJerseys','freeAgency','draft','trades','spawned'])u.offseasonState.events[k]=u.offseasonState.events[k]||[];}
  for(const off of u.offseasonHistory||[]){off.events=off.events||{};for(const k of ['coachMarket','transfers','retirements','declarations','hallOfFame','retiredJerseys','freeAgency','draft','trades','spawned'])off.events[k]=off.events[k]||[];}
...
function finishSeasonMutable(u,rng){
  ...
  u.postseasonState.complete=true;u.phase='Season Complete';u.offseasonState={year:u.year,stageIndex:0,started:false,complete:false,summary,events:{coachMarket:[],transfers:[],retirements:[],declarations:[],hallOfFame:[],retiredJerseys:[],freeAgency:[],draft:[],trades:[],spawned:[]},declaredIds:[]};
}
...
function careerStatBonus(p,league){const seasons=(p.stats?.seasons||[]).filter(s=>s.league===league),tot=k=>sum(seasons,s=>s[k]||0),cap=(v,m)=>Math.min(m,v);if(p.position==='QB')return cap(tot('passYards')/10000*9,30)+cap(tot('passTD')/100*7,22);if(['HB','FB'].includes(p.position))return cap(tot('rushYards')/4000*10,30)+cap(tot('rushTD')/40*7,18);if(['WR','TE'].includes(p.position))return cap(tot('recYards')/4000*10,30)+cap(tot('recTD')/40*7,18);if(['EDGE','DT','LB'].includes(p.position))return cap(tot('sacks')/40*12,32)+cap(tot('tackles')/400*8,18);if(['CB','S'].includes(p.position))return cap(tot('defInterceptions')/15*12,30)+cap(tot('tackles')/350*6,15);return Math.max(0,seasons.length-3)*2;}
function nflHofScore(p){const mvp=awardCount(p,'NFL MVP'),major=awardCount(p,a=>a.league==='NFL'&&['Offensive Player of the Year','Defensive Player of the Year'].includes(a.name)),all=awardCount(p,a=>a.league==='NFL'&&String(a.name).startsWith('All-NFL')),sb=titleCount(p,a=>a.league==='NFL'&&a.title==='Super Bowl'),conf=titleCount(p,a=>a.league==='NFL'&&String(a.title).endsWith('Champion')&&a.title!=='Division Champion'),div=titleCount(p,a=>a.league==='NFL'&&a.title==='Division Champion'),seasons=(p.stats?.seasons||[]).filter(s=>s.league==='NFL').length,rarity={Generational:22,Legend:13,Epic:5,Rare:1}[p.trueRarity]||0;return Math.round((mvp*45+major*22+all*8+sb*14+conf*4+div*1.5+Math.max(0,seasons-6)*2+careerStatBonus(p,'NFL')+rarity)*10)/10;}
function collegeHofScore(p){const heisman=awardCount(p,'Heisman Trophy'),major=awardCount(p,a=>a.league==='COLLEGE'&&['College Offensive Player of the Year','College Defensive Player of the Year'].includes(a.name)),all=awardCount(p,a=>a.league==='COLLEGE'&&String(a.name).startsWith('All-College')),titles=titleCount(p,a=>a.league==='COLLEGE'&&a.title==='National Championship'),seasons=(p.stats?.seasons||[]).filter(s=>s.league==='COLLEGE').length;return Math.round((heisman*50+major*20+all*10+titles*15+Math.max(0,seasons-2)*4+careerStatBonus(p,'COLLEGE'))*10)/10;}
function hofReasons(p,league){const names=[];if(league==='NFL'){const m=awardCount(p,'NFL MVP'),a=awardCount(p,x=>x.league==='NFL'&&String(x.name).startsWith('All-NFL')),s=titleCount(p,x=>x.league==='NFL'&&x.title==='Super Bowl');if(m)names.push(`${m}× MVP`);if(a)names.push(`${a}× All-NFL`);if(s)names.push(`${s}× Super Bowl`);}else{const h=awardCount(p,'Heisman Trophy'),a=awardCount(p,x=>x.league==='COLLEGE'&&String(x.name).startsWith('All-College')),n=titleCount(p,x=>x.league==='COLLEGE'&&x.title==='National Championship');if(h)names.push(`${h}× Heisman`);if(a)names.push(`${a}× All-College`);if(n)names.push(`${n}× National Champion`);}return names.slice(0,3).join(' · ')||'Sustained elite career';}
function processHallOfFame(u){const events=[];u.hallOfFame=u.hallOfFame||{NFL:[],COLLEGE:[]};const existingNFL=new Set(u.hallOfFame.NFL.map(x=>x.playerId)),existingCollege=new Set(u.hallOfFame.COLLEGE.map(x=>x.playerId));const nfl=u.players.filter(p=>p.retired&&!existingNFL.has(p.id)&&(p.retirementYear||u.year)<=u.year-4).map(p=>[p,nflHofScore(p)]).filter(([,score])=>score>=82).sort((a,b)=>b[1]-a[1]).slice(0,8);for(const[p,score]of nfl){const row={league:'NFL',playerId:p.id,year:u.year,score,reasons:hofReasons(p,'NFL')};u.hallOfFame.NFL.push(row);p.hallOfFame={...(p.hallOfFame||{}),NFL:u.year};events.push(row);addNews(u,'HALL OF FAME',86,`${p.name} enters the Pro Football Hall of Fame`,row.reasons,null,p.id);}const college=u.players.filter(p=>!existingCollege.has(p.id)).map(p=>{const yrs=(p.stats?.seasons||[]).filter(s=>s.league==='COLLEGE').map(s=>s.year),last=yrs.length?Math.max(...yrs):(p.draftYear?Math.max(1,p.draftYear-1):null);return[p,last,collegeHofScore(p)]}).filter(([,last,score])=>last&&u.year-last>=4&&score>=62).sort((a,b)=>b[2]-a[2]).slice(0,10);for(const[p,,score]of college){const row={league:'COLLEGE',playerId:p.id,year:u.year,score,reasons:hofReasons(p,'COLLEGE')};u.hallOfFame.COLLEGE.push(row);p.hallOfFame={...(p.hallOfFame||{}),COLLEGE:u.year};events.push(row);addNews(u,'HALL OF FAME',72,`${p.name} enters the College Football Hall of Fame`,row.reasons,p.collegeId,p.id);}return events;}
function teamRetirementScore(p,teamId,league){const seasons=(p.stats?.seasons||[]).filter(s=>s.league===league&&s.teamId===teamId),years=new Set(seasons.map(s=>s.year)),awards=(p.awards||[]).filter(a=>a.league===league&&years.has(a.year)),titles=(p.teamTitles||[]).filter(a=>a.league===league&&a.teamId===teamId);let score=seasons.length*2;if(league==='NFL'){score+=awards.filter(a=>a.name==='NFL MVP').length*25+awards.filter(a=>['Offensive Player of the Year','Defensive Player of the Year'].includes(a.name)).length*12+awards.filter(a=>String(a.name).startsWith('All-NFL')).length*6+titles.filter(a=>a.title==='Super Bowl').length*15+titles.filter(a=>['AFC Champion','NFC Champion'].includes(a.title)).length*5+titles.filter(a=>a.title==='Division Champion').length*1.5;score+=p.trueRarity==='Generational'?10:p.trueRarity==='Legend'?5:0;}else{score+=awards.filter(a=>a.name==='Heisman Trophy').length*35+awards.filter(a=>String(a.name).startsWith('All-College')).length*8+titles.filter(a=>a.title==='National Championship').length*15;}return Math.round(score*10)/10;}
function processRetiredJerseys(u,retirements=[]){const events=[],already=new Set((u.retiredJerseys||[]).map(x=>`${x.teamId}-${x.playerId}`));for(const r of retirements){const p=playerById(u,r.playerId);if(!p)continue;const teamIds=[...new Set((p.stats?.seasons||[]).map(s=>s.teamId).filter(Boolean))];if(p.collegeId)teamIds.push(p.collegeId);for(const teamId of [...new Set(teamIds)]){const t=teamById(u,teamId);if(!t||!['NFL','COLLEGE'].includes(t.league)||already.has(`${teamId}-${p.id}`))continue;const score=teamRetirementScore(p,teamId,t.league),threshold=t.league==='NFL'?55:48;if(score<threshold)continue;const row={year:u.year,league:t.league,teamId,playerId:p.id,number:p.jerseyNumber||jerseyNumber(p),score};u.retiredJerseys.push(row);t.retiredJerseys=t.retiredJerseys||[];t.retiredJerseys.push(row);already.add(`${teamId}-${p.id}`);events.push(row);addNews(u,'RETIRED JERSEY',80,`${t.name} retires No. ${row.number} for ${p.name}`,`A rare franchise honor after a career legacy score of ${score}.`,t.id,p.id);}}return events;}
...
export function advanceOffseasonStage(universe){const u=ensureUniverse(cloneUniverse(universe)),rng=makeRng(u.rngState||u.seed);if(u.phase==='Season Complete'){u.phase='Offseason';u.offseasonState.started=true;}if(u.phase!=='Offseason'){u.rngState=rng.state();return u;}const st=u.offseasonState,idx=st.stageIndex,stage=OFFSEASON_STAGES[idx];if(!stage){st.complete=true;u.phase='Ready for Next Season';u.rngState=rng.state();return u;}
  ...
  else if(stage==='Trades & UFL'){st.events.trades=tradeMarket(u,rng);trimNFLRosters(u);}
